...
fn main() {
    ...
    let result = match command {
        ...
        "creature-init" => creature_init_json(&args),
...
fn help() -> Result<(), String> {
    ...
    println!("  creature-init --json          create or validate the local .creature workspace");
...
fn creature_init_json(args: &[String]) -> Result<(), String> {
    if !args.iter().any(|arg| arg == "--json") {
        return Err("creature-init requires --json".to_string());
    }
    let workspace = Path::new(".creature");
    let created = !workspace.exists();
    fs::create_dir_all(workspace)
        .map_err(|err| format!("failed to create {}: {err}", workspace.display()))?;
    let status = if created { "created" } else { "validated" };
    let value = serde_json::json!({
        "workspace_path": workspace.display().to_string(),
        "status": status,
        "authority_note": "The .creature workspace is local runtime state. Git remains the outer memory; LLM output remains advisory."
    });
    println!(
        "{}",
        serde_json::to_string_pretty(&value).map_err(|err| err.to_string())?
